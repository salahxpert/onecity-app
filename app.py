...
class GoogleSheetSync:
    ...
    # ===== write_to_sheet (FULLY FIXED) =====
    def write_to_sheet(self, table_name, row_data, sheet_name=None):
        """Write or update a row in Google Sheets with exact header mapping."""
        if not self.client:
...
# ===== ACCOUNTS (with Salary & Conveyance) =====
@app.route('/accounts')
@login_required
def accounts():
    conn = get_db()
...
# ===== add_collection with FULL AUTO-POSTING =====
