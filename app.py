...
class GoogleSheetSync:
    ...
    def _authenticate(self):
        """Authenticate using Environment Variable first, fallback to file."""
        try:
            # 1. Try Environment Variable
            creds_json = os.getenv('GOOGLE_CREDENTIALS')
            if creds_json:
                logger.info("Using credentials from Environment Variable: GOOGLE_CREDENTIALS")
                creds_dict = json.loads(creds_json)
                self.creds = ServiceAccountCredentials.from_json_keyfile_dict(creds_dict, self.scope)
                self.client = gspread.authorize(self.creds)
                logger.info("✅ Google Sheets authenticated via Environment Variable")
                return
            
            # 2. Fallback: credentials.json file
            if os.path.exists('credentials.json'):
                ...
                self.client = gspread.authorize(self.creds)
                logger.info("✅ Google Sheets authenticated via file")
                return
            
            raise Exception("No credentials found in Environment Variable or credentials.json file")
            
        except json.JSONDecodeError as e:
            logger.error(f"❌ Invalid JSON in GOOGLE_CREDENTIALS: {e}")
            raise Exception("Invalid JSON in GOOGLE_CREDENTIALS environment variable")
        except Exception as e:
